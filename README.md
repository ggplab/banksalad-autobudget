...
뱅크샐러드 가계부를 자동으로 정리한다. 앱에서 export 한 번 보내면, 적재부터 리포트 전달까지 사람 손이 안 간다.

<p align="center"><img src="docs/images/card-pipeline.png" width="520" alt="실행이 끝나면 Discord 에 결과 카드 한 장이 온다"></p>

## 1. 목적

뱅크샐러드 카드 내역은 가맹점 이름뿐이다. "쿠팡(쿠페이) 8,400원"만 보고는 이유식인지 세제인지 모른다.
이 파이프라인은 그 빈칸을 채운다.
...
n8n 으로 운영하던 것을 n8n 없이도 돌아가게 정리한 공개판이다. 사람이 하는 일은 뱅샐 앱에서
"설정 → 데이터 내보내기 → 파일로 받기" 한 번이다.

## 2. 워크플로우

Trigger 하나, Task 여섯, Target 하나. Task 하나가 명령 하나다.
